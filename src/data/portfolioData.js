export const portfolioData = {
  site: {
    name: "Septada",
    role: "IT Student",
    email: "lvntda@gmail.com",
    github: "https://github.com/alvn168",
    linkedin: "https://www.linkedin.com/in/septada-alvian-rasyid-598334271/",
    location: "Indonesia",
  },
  hero: {
    greeting: "Hello There!",
    name: "SepTada",
    firstName: "Sep",
    lastName: "Tada",
    titles: [
      "IT Student",
      "Fullstack Developer",
      "Tech Explorer"
    ],
    description: "Passionate about building clean, maintainable, and responsive web applications while continuously learning new technologies.",
  },
  about: {
    description: "My name is Septada Alvian, an IT student passionate about software development. I enjoy building clean, maintainable, and responsive web applications while continuously improving my skills in modern technologies. Currently, I am expanding my knowledge in Cloud Computing, server administration, and software architecture, with a strong interest in creating scalable and reliable applications.",
    techStack: [
      { category: "Tech Stack", items: ["React", "Node.js", "Express", "MySQL", "JavaScript (ES6+)", "Tailwind CSS", "HTML5", "CSS3"] },
      { category: "Tools", items: ["Git", "GitHub", "VS Code", "Figma", "Vite"] },
    ]
  },
  projects: [
    {
      id: 1,
      title: "Management Academic",
      description: "A modern academic management website, featuring a responsive design, clean user interface, and seamless user experience..",
      thumbnail: "/thumbnails/management-academic.jpg",
      techStack: ["React", "Tailwind CSS", "JavaScript"],
      githubUrl: "",
      liveDemoUrl: "https://academic.tada.my.id/",
      featured: true,
      status: "Preview",
    },
    {
      id: 2,
      title: "Customer Churn",
      description: "An end-to-end Data Science capstone project that predicts customer churn using Machine Learning. Features EDA, SMOTE balancing, hyperparameter tuning, and an interactive Streamlit dashboard for real-time predictions on a 15,000-customer dataset.",
      thumbnail: "/thumbnails/customer-churn.png",
      techStack: ["Python", "Streamlit"],
      githubUrl: "https://github.com/popzassou/Data-Science-Capstone",
      liveDemoUrl: "https://data-science-capstone-customer-churn.streamlit.app/",
      featured: true,
      status: "Preview",
    },
    {
      id: 3,
      title: "Personal Portfolio",
      description: "Modern personal portfolio website built using React and Tailwind CSS.",
      thumbnail: "",
      techStack: ["React", "Tailwind CSS", "JavaScript"],
      githubUrl: "#",
      liveDemoUrl: "#",
      featured: true,
      status: "coming-soon",
    },

  ],

  navLinks: [
    { label: 'Home', href: '#home' },
    { label: 'About', href: '#about' },
    { label: 'Projects', href: '#projects' },
    { label: 'Contact', href: '#contact' },
  ]
};
